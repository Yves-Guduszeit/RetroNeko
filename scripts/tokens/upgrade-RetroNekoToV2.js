...
async function main() {
    ...
    await enableTakeFees(retroNekoProxy);
    await enableSwap(retroNekoProxy);

    //TODO: Add Liquidity + enable trading
}

async function upgradeRetroNekoViaProxy(retroNekoProxyAddress) {
    console.log("Upgrading RetroNeko via Proxy ...");
    const RetroNeko = await ethers.getContractFactory("contracts/tokens/RetroNekoV2.sol:RetroNeko");
